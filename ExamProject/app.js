const apiKey = 'df40fdba408e931ec588f094600831bb';
let nameCity ="";
document.addEventListener('DOMContentLoaded', async function () {
    await updateCityWeather('Львів', 49.8397, 24.0297);
    await updateCityWeather('Харків', 49.9935, 36.2304);
    await updateCityWeather('Київ', 50.2700, 30.3125);
    await updateCityWeather('Львів', 49.5033, 24.0156);
    await updateCityWeather('Чернівці', 48.1727, 25.5604);
    await updateCityWeather('Запоріжжя', 49.3734, 11.4186);
    handleTodayClick();
});
const weatherTranslations = {
    'clear sky': 'чисте небо',
    'few clouds': 'трішки хмаро',
    'scattered clouds': 'помірно хмарно',
    'broken clouds': 'великі хмари',
    'overcast clouds': 'дуже хмарно',
    'mist': 'туман',
...
function getLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            updateWeatherName('Ivano-Frankivsk');
        });
    } else {
...
async function updateOtherCitiesForecast() {
    const otherCitiesContainer = document.querySelector('#otherCitiesForecast');
    otherCitiesContainer.innerHTML = '';

    const cities = ['Львів', 'Чернівці', 'Запоріжжя']; // Додайте інші міста, якщо потрібно
    ...
    for (const city of cities) {
        ...
        if (coordinates) {
            ...
            console.error(`Could not find coordinates for the city: ${city}`);
        }
    }
}
